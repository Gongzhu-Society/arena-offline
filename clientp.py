...
from pnetinterface import PNet

class MrP(Agent):
    def __init__(self):
        super(Agent, self).__init__()
        new_pnet = PNet()
        new_pnet.load_w("robot-net-p-v1.txt")
        self.pnet = new_pnet
    def pick_a_card(self):
        if len(self.cards_on_table)==0:
            color = 'A'
        else:
            color = self.cards_on_table[0][0]
        statevec = self.pnet.to_state_li(self.history, self.place, self.cards_on_table, self.initial_cards)
        self.pnet.play_one_card('not important', statevec, self.cards,
                      [], color, False)
